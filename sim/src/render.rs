...
impl Renderer {
    ...
    /// agrupando los vértices en pocos `draw_mesh` para eficiencia. Como
    /// triángulos orientados por la velocidad si `orient_to_velocity`, o como
    /// discos texturizados en otro caso.
    pub fn draw_particles(&mut self, sim: &Simulation, params: &SimParams) {
        let alpha = params.brightness.clamp(0.0, 1.0);

        if params.orient_to_velocity {
            // Flechas: un triángulo por partícula, apuntando según su velocidad.
